...
yaml_file <- "data/personal.yml"

personal_list <- read_yml_and_check_levels(yaml_file)

render_title(personal_list, "personal")

config_personal <- yaml::read_yaml("config/personal.yml")

map(config_personal, ~ render_singletons_yml(.x, list_from_yaml = personal_list)) |> invisible()
```
...
yaml_file <- "data/chronology.yml"

chronology_list <- read_yml_and_check_levels(yaml_file)

render_title(chronology_list, "chronology")

config_chronology <- yaml::read_yaml("config/chronology.yml")

map(config_chronology, ~ render_singletons_yml(.x, list_from_yaml = chronology_list)) |> invisible()
```

<!-- SUMMARY -->

``` {r, results="asis", echo=F}
yaml_file <- "data/summary.yml"

summary_list <- read_yml_and_check_levels(yaml_file)

render_title(summary_list, "summary")

config_summary <- yaml::read_yaml("config/summary.yml")

map(config_summary, ~ render_singletons_yml(.x, list_from_yaml = summary_list)) |> invisible()
```

<!-- Personal Links: -->

``` {r, results="asis", echo=F}
yaml_file <- "data/links.yml"

links_list <- read_yml_and_check_levels(yaml_file)

render_title(links_list, "links")

config_links <- yaml::read_yaml("config/links.yml")

map(config_links, ~ render_singletons_yml(.x, list_from_yaml = links_list)) |> invisible()
```


<!-- DEVELOPMENT PERIOD -->

``` {r devel, results="asis", echo=F, eval=T}
yaml_file <- "data/development.yml"

links_list <- read_yml_and_check_levels(yaml_file)

render_title(links_list, "development")

config_links <- yaml::read_yaml("config/development.yml")

map(links_list, \(y) {
  map(config_links, \(x) render_singletons_yml(x, list_from_yaml = y)) |> invisible()
}) |> invisible()
```
